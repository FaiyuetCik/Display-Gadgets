...
static uint16_t rgb565(uint8_t r, uint8_t g, uint8_t b) {
  // R/B swapped for 0.96" panel (BGR panel order)
...
// ========================= LCD init (Arduino_GFX software SPI) =========================
...
static void initDisplay() {
  ...
  Serial.print("LCD w=");  Serial.print(gfx->width());
  Serial.print(" h=");     Serial.println(gfx->height());

  gridOriginX = (gfx->width()  - GRID_W * CELL_SIZE) / 2;
  gridOriginY = (gfx->height() - GRID_H * CELL_SIZE) / 2;
}
...
static void showMessage(const char *title, const char *line) {
  gfx->fillScreen(C_BLACK);
  gfx->setTextColor(rgb565(255, 220, 80), C_BLACK);
  gfx->setTextSize(1);
  gfx->setCursor(4, 44);  gfx->print(title);
  gfx->setTextColor(C_WHITE, C_BLACK);
  gfx->setCursor(8, 72);  gfx->print(line);
}
...
static void drawCell(int8_t cx, int8_t cy, uint16_t color) {
  ...
  gfx->fillRect(px, py, CELL_SIZE - 2, CELL_SIZE - 2, color);
}
