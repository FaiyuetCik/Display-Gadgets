...
static void rawTouchToScreen(uint16_t rawX, uint16_t rawY, int &screenX, int &screenY) {
  ...
  if (screenY >= SCREEN_H) screenY = SCREEN_H - 1;
}

// ========================= Drawing =========================

static void drawClearBar() {
  tft.fillRect(0, CLEAR_ZONE_TOP, SCREEN_W, SCREEN_H - CLEAR_ZONE_TOP, C_DARKGREY);
  tft.drawFastHLine(0, CLEAR_ZONE_TOP, SCREEN_W, C_WHITE);
  tft.setTextDatum(MC_DATUM);
  tft.setTextColor(C_WHITE, C_DARKGREY);
  tft.drawString("CLEAR", SCREEN_W / 2, CLEAR_ZONE_TOP + 18, 2);
}
...
static void updateCircleCount() {
  tft.fillRect(8, 46, SCREEN_W - 8, 12, C_BLACK);
  tft.setTextDatum(TL_DATUM);
  char buf[32];
  snprintf(buf, sizeof(buf), "Circles: %d", g_circleCount);
  tft.setTextColor(C_GREEN, C_BLACK);
