/*
  XIAO ESP32-S3 Plus 1.14 Inch Display graphic test.

  This sketch uses the same Seeed_GFX / TFT_eSPI setup as
  example/basic/xiao_esp32s3_114_display, then runs a compact graphics benchmark:
    - color bars
    - lines
    - rectangles
    - circles
...
static void showTitle(const char *title) {
  ...
  tft.drawString("XIAO ESP32-S3 Plus", tft.width() / 2, 118, 1);
  delay(650);
}
