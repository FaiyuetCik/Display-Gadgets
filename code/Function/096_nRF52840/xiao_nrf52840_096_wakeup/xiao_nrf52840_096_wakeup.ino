/*
  ...
  Required libraries:
    - Arduino_GFX (GFX_Library_for_Arduino)
...
// ========================= Colors =========================
// 0.96" panel has R/B hardware swap — define visual aliases
...
static constexpr uint16_t C_RED    = 0xF800;   // wire-level red
static constexpr uint16_t C_BLUE   = 0x001F;   // wire-level blue
static constexpr uint16_t C_GRAY   = 0x8410;
static constexpr uint16_t C_LINE   = 0x8410;  // medium gray, visible on BGR panel

// Visual aliases: what the eye actually sees on this BGR panel
...
static bool initDisplay() {
  ...
  gfx = new Arduino_ST7789(bus, LCD_RST_PIN, 2, true, 80, 160, 24, 0, 24, 0);
...
static void drawStaticAwakeUi() {
  ...
  gfx->drawFastHLine(5, 92, 70, V_WHITE);
  ...
  gfx->drawFastHLine(5, 141, 70, V_WHITE);
...
static void updateAwakeUi() {
  ...
  printTextFixed(16, 115, V_WHITE, acc, 15);

  String gyr = fmtAxis(g_gx) + " " + fmtAxis(g_gy) + " " + fmtAxis(g_gz);
  printTextFixed(16, 128, V_WHITE, gyr, 15);
}
