/*
  ...
  Factory Dashboard v1.11 - 0.96 small-font IMU field redraw fix + uppercase TAP
  ...
  Board difference:
    ...
    - Backlight is ON/OFF, matching nRF52840 0.96 behavior
    - BAT divider:
...
static void handleButtonActions() {
  ...
  if (g_btnBPendingAction) {
    ...
    // USR2: only toggle Hello / Seeed. Do not reset Tap.
...
static void printHeader() {
  Serial.println();
  Serial.println("=== XIAO ESP32-S3 PLUS 0.96 Factory Dashboard v1.13.1 D16-two-line-yellow ===");
  ...
  Serial.printf("[BTN] USR1=D6(%d) BL ON/OFF, USR2=D7(%d) Header toggle\n", BTN_A_PIN, BTN_B_PIN);
}
