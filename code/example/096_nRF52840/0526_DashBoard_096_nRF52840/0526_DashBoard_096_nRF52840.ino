/*
  ...
  Factory Dashboard v1.2.6 - Tiny UI
  ...
  Board difference vs 1.14 / 1.47:
    ...
    - IMU INT uses D9 on the 0.96 PRD pin map
    - Backlight is treated as ON/OFF. The 0.96/1.14 hardware did not show useful PWM dimming.
...
static constexpr uint8_t IMU_INT_PIN   = D9;   // 0.96 PRD: IMU_INT
...
static void drawStaticLayout() {
  ...
  gfx->setCursor(SECTION_X0 + 1, Y_ACC); gfx->print("A");
  gfx->setCursor(SECTION_X0 + 1, Y_GYR); gfx->print("G");

    gfx->drawFastHLine(5, 147, 70, V_WHITE);
  drawTinyText(6, ROW_FOOT, "XIAO nRF52840+", V_YELLOW);
...
static void updateUiFast() {
  ...
  if (tap != cache_tap) {
    cache_tap = tap;
    printTextFixed(48, Y_TAP, V_YELLOW, tap, 6);
  }
...
static void updateUiSlow() {
  ...
  if (g_imuOk) {
    ...
    if (acc != cache_acc) {
      cache_acc = acc;
      printTextFixed(SECTION_X0 + 1, Y_ACC, V_WHITE, acc, 11);
    }

    String gyr = String("G ") + fmtAxisInt(g_gx / 10.0f) + " " + fmtAxisInt(g_gy / 10.0f) + " " + fmtAxisInt(g_gz / 10.0f);
    if (gyr != cache_gyr) {
      cache_gyr = gyr;
      printTextFixed(SECTION_X0 + 1, Y_GYR, V_WHITE, gyr, 11);
    }
  } else {
    if (cache_acc != "NOIMU") {
      cache_acc = "NOIMU";
      cache_gyr = "";
      printTextFixed(SECTION_X0 + 1, Y_ACC, V_RED, "IMU NO", 11);
      printTextFixed(SECTION_X0 + 1, Y_GYR, V_RED, "", 11);
    }
  }
}
...
void setup() {
  ...
  Serial.println("[BTN] USR1=BL ON/OFF, USR2=Header Hello/Seeed");
