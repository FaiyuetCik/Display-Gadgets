/*
  ...
  Factory Dashboard v1.2.4 - I2C hot-plug tolerant monitor

  Based on:
    - 0522_DashBoard_114_nRF52840.ino
    - 0521_DashBoard_114_ESP32.ino
    - completed ESP32-S3 1.47 Dashboard BAT state-machine path

  Goals:
    - Keep the nRF52840 Plus 1.14 Dashboard visual style and card layout.
    - Keep ESP32-S3 peripherals from the previous 0521 bring-up:
        LCD, Grove I2C scan, IMU, PDM mic, USR1/USR2/USR3.
    - Add ESP32-S3 D16 voltage sense display:
        D16 ADC voltage + calculated external voltage through divider ratio.
        This is voltage reference only, not battery-presence or charge-state detection.

  1.14 board:
    - LCD: ST7789 135x240, IPS=true, offset 52/40/53/40
    - No Touch
    - No SD
    - Grove I2C
    - 3 user buttons:
        USR1: cycle brightness 100/75/50/25/0%
        USR2: screen off / restore brightness
        USR3: toggle title Hello,XIAO! / Seeed
*/
...
static constexpr uint32_t I2C_SCAN_MS = 8;     // one address per step; full 0x08-0x77 sweep is about 0.9 s
static constexpr uint32_t I2C_SETTLE_MS = 180; // require SDA/SCL high continuously before resuming after hot-plug
static constexpr uint32_t I2C_NOTICE_MS = 700; // briefly show that a bus change was detected
static constexpr uint32_t I2C_CLOCK_HZ = 100000; // safest common speed for unknown Grove I2C devices
...
enum I2cMonitorPhase {
  I2C_MONITOR_SCAN = 0,
  I2C_MONITOR_SETTLE
};

I2cMonitorPhase g_i2cMonitorPhase = I2C_MONITOR_SCAN;
bool g_i2cAccessAllowed = true;
uint32_t g_i2cLinesHighSinceMs = 0;
uint32_t g_i2cNoticeUntilMs = 0;
uint8_t g_i2cLastFaultAddr = 0;
uint8_t g_i2cLastFaultCode = 0;
...
static bool i2cWrite8(uint8_t addr, uint8_t reg, uint8_t val) {
  if (!g_i2cAccessAllowed) return false;
...
static bool initLcd() {
  // Keep the backlight off while the ST7789 is being reset and initialized.
  // This avoids showing a long black flash if the MCU restarts after a hot-plug transient.
  pinMode(LCD_BL_PIN, OUTPUT);
...
static bool i2cDeviceListsEqual(const uint8_t *a, uint8_t aCount,
                                const uint8_t *b, uint8_t bCount) {
...
static bool i2cLinesReleased() {
  // Read the pin levels without changing the ESP32 pin matrix configuration.
  return gpio_get_level((gpio_num_t)I2C_SDA_PIN) == 1 &&
         gpio_get_level((gpio_num_t)I2C_SCL_PIN) == 1;
}
...
static void beginI2cSettle(uint8_t addr, uint8_t err) {
  // Fail open: stop all I2C traffic, but leave LCD, mic, buttons and UI alive.
  // Keep the previous stable address list on screen until a clean sweep completes.
  g_i2cMonitorPhase = I2C_MONITOR_SETTLE;
  g_i2cAccessAllowed = false;
  g_i2cLinesHighSinceMs = 0;
  g_i2cNextCount = 0;
  g_i2cScanAddr = 0x08;
  g_i2cScanFault = true;
  g_i2cLastFaultAddr = addr;
  g_i2cLastFaultCode = err;
  g_i2cNoticeUntilMs = millis() + I2C_NOTICE_MS;

  if (g_i2cScanFaultCount < 255) g_i2cScanFaultCount++;

  Serial.printf("[I2C] hot-plug/bus fault addr=0x%02X err=%u; pause bus, keep UI alive\n",
                addr, err);
}

static void serviceI2cSettle() {
  uint32_t now = millis();
  ...
  if (g_i2cLinesHighSinceMs == 0) {
    g_i2cLinesHighSinceMs = now;
    return;
  }

  if (now - g_i2cLinesHighSinceMs < I2C_SETTLE_MS) return;

  // Do not Wire.end() or toggle SDA/SCL here. Those aggressive recovery methods
  // can create additional pin/power disturbances. A released bus is simply
  // resumed at the conservative universal speed.
  Wire.setClock(I2C_CLOCK_HZ);
  Wire.setTimeOut(8);

  g_i2cAccessAllowed = true;
  g_i2cMonitorPhase = I2C_MONITOR_SCAN;
  g_i2cScanFault = false;
  g_i2cNextCount = 0;
  g_i2cScanAddr = 0x08;
  g_i2cLinesHighSinceMs = 0;

  Serial.println("[I2C] lines stable; resume clean address sweep");
}

static void commitI2cScan() {
  bool changed = !i2cDeviceListsEqual(g_i2cDevices, g_i2cCount,
                                      g_i2cNextDevices, g_i2cNextCount);
  ...
  if (changed) {
    ...
    cache_grove = ""; // force the I2C row to redraw on the next UI tick
    g_i2cNoticeUntilMs = millis() + I2C_NOTICE_MS;
    printI2cScanResult();
  }
...
static void scanI2cBus() {
  if (g_i2cMonitorPhase == I2C_MONITOR_SETTLE) {
    serviceI2cSettle();
    return;
  }

  // Catch connector bounce before starting another I2C transaction.
  if (!i2cLinesReleased()) {
    beginI2cSettle(g_i2cScanAddr, 0xFE); // synthetic code: physical line held low
    return;
  }
  ...
  // Exactly one address is probed per call. This bounds the blocking time of
  // the monitor and prevents an entire scan burst from starving the LCD UI.
  ...
  if (err == 0) {
    ...
    // ESP32 Wire commonly reports 4/5 for bus error or timeout.
    beginI2cSettle(addr, err);
  }
  // err=2/3 is a normal NACK during scanning and is intentionally ignored.
}
...
static void updateUiFast() {
  ...
  if (millis() - g_lastVsenseUiMs >= VSENSE_UI_MS || cache_bat.length() == 0) {
    ...
    if (needDraw && g_lcdOk) {
      ...
      acquireForLcd();
      gfx->fillRect(VSENSE_ROW_X, ROW_BAT - 2, VSENSE_ROW_W, 14, C_BLACK);
      gfx->setTextSize(1);
      gfx->setTextColor(C_YELLOW, C_BLACK);
      gfx->setCursor(VSENSE_ROW_X, ROW_BAT);
      gfx->print(padRight(rowText, VSENSE_ROW_CHARS));
  ...
  String groveText;
  uint16_t groveColor = C_CYAN;

if (g_i2cMonitorPhase == I2C_MONITOR_SETTLE) {
  groveText = "detecting";
  groveColor = C_YELLOW;
} else if (g_i2cCount == 0) {
  groveText = "none";
} else {
  char t[24];
  snprintf(t, sizeof(t), "%u 0x%02X",
           g_i2cCount,
           g_i2cDevices[0]);
  groveText = String(t);
}

  if (millis() < g_i2cNoticeUntilMs && g_i2cMonitorPhase != I2C_MONITOR_SETTLE) {
    groveColor = C_GREEN;
  }

  String groveCacheKey = groveText + ":" + String(groveColor);
  if (groveCacheKey != cache_grove) {
    cache_grove = groveCacheKey;
...
static void printHeader() {
  Serial.println();
  Serial.println("=== XIAO ESP32-S3 Plus 1.14 Factory Dashboard v1.2.4 I2C-hotplug ===");
...
void setup() {
  ...
  // Draw the product UI immediately. The previous 900 ms startup delay made
  // any brownout/reset caused by hot-plug look like a long black-screen fault.
  initLcd();
  drawStaticLayout();
  applyBacklight();
  ...
  Wire.setClock(I2C_CLOCK_HZ);
  Wire.setTimeOut(8);      // one failed probe can block for at most about 8 ms
  ...
  Serial.println("[BOOT] 1.14 ESP32-S3 dashboard v1.2.4 I2C-hotplug ready");
}
