/*
  ...
  IMU D14 Wake / System ON Sleep Test v0.1

  Target behavior:
    Screen sleeps -> nRF52840 enters System ON sleep -> user picks up / moves device
    -> IMU INT1 triggers D14 -> CPU wakes -> screen lights up.

  This is a standalone validation firmware for the 1.14 nRF52840 Plus display board.

  Pin map, based on the corrected 1.14 nRF52840 Plus dashboard:
    LCD CS   = D2
    LCD DC   = D3
    I2C SDA  = D4
    I2C SCL  = D5
    ...
    IMU INT  = D14
    LCD RST  = D17
    LCD BL   = D18  backlight on/off
    USR3     = D19  reset counters

  Notes:
    - This uses nRF System ON sleep, not System OFF.
    - RAM/state are preserved and wake is fast.
    - USB connection itself will dominate current, so for current testing use battery power.
    - Backlight is controlled as ON/OFF here because 1.14 hardware does not show useful PWM dimming.
*/
...
static void drawAwakeLayout() {
  ...
  gfx->setCursor(6, 22);
  gfx->print("1.14 IMU D14 Wake");

  gfx->drawFastHLine(6, 34, 123, C_LINE);
  ...
  gfx->setTextColor(C_WHITE, C_BLACK);
  gfx->setCursor(17, 116);
  gfx->print("A");
  gfx->setCursor(17, 132);
  gfx->print("G");
  gfx->setCursor(17, 146);
  gfx->print("INT");

  drawCard(4, 166, 127, 41, C_BLUE, "TEST");
  gfx->setTextColor(C_WHITE, C_BLACK);
  gfx->setCursor(17, 186);
  gfx->print("U1 Sleep  U2 Wake");

  gfx->setTextColor(C_GRAY, C_BLACK);
  gfx->setCursor(19, 222);
...
static void drawSleepHintThenOff() {
  ...
  gfx->print("System ON sleep");

  gfx->setTextColor(C_YELLOW, C_BLACK);
  gfx->setCursor(10, 122);
  gfx->print("Move / pick up to wake");

  gfx->setTextColor(C_GRAY, C_BLACK);
  gfx->setCursor(10, 150);
  gfx->print("wake source: IMU D14");
}
