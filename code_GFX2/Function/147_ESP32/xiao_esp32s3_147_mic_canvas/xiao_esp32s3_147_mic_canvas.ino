...
static void drawBarFrame() {
  display.drawRect(BAR_X - 2, BAR_Y_TOP - 2, BAR_W + 4, BAR_H + 4, C_WHITE);
}
...
static void drawBar(float vol) {
  ...
  for (int i = 0; i < SEGS; i++) {
    ...
    if (i < lit) {
      ...
      display.fillRect(BAR_X, y0, BAR_W, SEG_H, C_BLACK);
    }
  }
...
static void drawLabel(float vol) {
  ...
  display.fillRect(0, BAR_Y_TOP - 40, SCREEN_W, 36, C_BLACK);
...
static void drawTitle() {
  ...
  display.setTextColor(C_DARKGREY, C_BLACK);
