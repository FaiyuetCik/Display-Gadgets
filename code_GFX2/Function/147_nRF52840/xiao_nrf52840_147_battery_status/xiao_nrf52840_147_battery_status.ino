/*
  XIAO nRF52840 Plus 1.47-inch Display - Battery Status Demo
  Library: Seeed_GFX2
...
static void updateBattery() {
  ...
  bool insertCandidate = usbNow && stableBatch && (baselineDrop || riseFromLowAbsent);
  ...
  if (insertCandidate) {
    ...
    g_batFilterState = riseFromLowAbsent ? "INS_LOW" : "INS_DROP";

    if (g_insertCandidateStreak >= BAT_INSERT_CONFIRM_COUNT) {
      measured.valid = true;
      measured.charging = chargingNow;
      confirmBatteryPresent(measured, riseFromLowAbsent ? "INSERT_LOW" : (chargingNow ? "INSERT_CHG" : "INSERT"));
      return;
    }
  } else {
    g_insertCandidateStreak = 0;
