 ...
 * I2C:     SDA=D4, SCL=D5; LSM6DS3 address=0x6A.
...
// Arduino's automatic prototype generator scans functions before it sees the
// full particle definition below. Keep this forward declaration above it.
struct Particle;
...
static uint8_t readImuWhoAmI() {
  Wire.beginTransmission(0x6A);
  Wire.write(0x0F);  // WHO_AM_I
  if (Wire.endTransmission(false) != 0) return 0x00;
  if (Wire.requestFrom((uint8_t)0x6A, (uint8_t)1) != 1) return 0x00;
  return Wire.read();
}
...
void setup() {
  ...
  // The Seeeduino nRF52 Wire implementation selects the XIAO D4/D5 pins from
  // Wire.begin(); it does not support the ESP32-style begin(SDA, SCL) overload.
  Wire.begin();
  delay(50);
  scanI2C();
  ...
  if (ret != 0) {
    uint8_t whoAmI = readImuWhoAmI();
    Serial.printf("[IMU] direct WHO_AM_I=0x%02X\n", whoAmI);

    // Seeed's library may report a transient read error on nRF52 even when
    // the device responds correctly. Only continue when the chip ID is valid.
    if (whoAmI == 0x69 || whoAmI == 0x6A) {
      Serial.println("[IMU] direct check OK; continuing");
    } else {
